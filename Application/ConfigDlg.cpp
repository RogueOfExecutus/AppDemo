...
BOOL ConfigDlg::OnInitDialog()
{
	...
	{
		int tempInt = GetPrivateProfileInt(_T("log"), _T("LogLevel"), 20000, parameter);
		...
		logList.SetCurSel(tempInt / 10000);
		tempInt = GetPrivateProfileInt(_T("config"), _T("EOFCR"), 0, parameter);
		checkIsCR.SetCheck(tempInt);
	}
	else
	{
